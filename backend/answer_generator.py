...
def build_answer_messages(question: str, evidence: str, history: list | None = None) -> list:
    ...
    messages.append(HumanMessage(content=ANSWER_USER_TEMPLATE.format(question=question, evidence=evidence)))
    return messages


def generate_answer(question: str, evidence: str, history: list | None = None) -> tuple[str, dict]:
    response = model.invoke(build_answer_messages(question, evidence, history))
...
async def stream_answer(question: str, evidence: str, history: list | None = None) -> AsyncIterator[tuple[str, dict]]:
    usage = {}
    async for chunk in model.astream(build_answer_messages(question, evidence, history)):
